...
BEGIN
  ...
  IF waiting_count >= cfg.lock_wait_count_threshold THEN
    PERFORM monitoring.post_alert(
      category := 'Lock Waits',
      severity := 'warning',
      message := pg_catalog.format('%s sessions waiting on locks (threshold %s)', waiting_count, cfg.lock_wait_count_threshold),
      details := pg_catalog.jsonb_build_object('waiting_count', waiting_count, 'threshold', cfg.lock_wait_count_threshold)
    );
  END IF;
END;
$$;
...
BEGIN
  ...
  IF delta >= cfg.deadlock_delta_threshold THEN
    PERFORM monitoring.post_alert(
      category := 'Deadlocks',
      severity := 'critical',
      message := pg_catalog.format('Deadlocks increased by %s (threshold %s)', delta, cfg.deadlock_delta_threshold),
      details := pg_catalog.jsonb_build_object('delta', delta, 'current_deadlocks', cur_deadlocks)
    );
  END IF;
...
BEGIN
  ...
  SELECT pg_catalog.pg_database_size(pg_catalog.current_database()) INTO size_bytes;
  ...
  growth_mb := pg_catalog.round(((size_bytes - st.last_database_size_bytes) / 1048576.0)::numeric, 2);

  IF growth_mb >= cfg.storage_growth_threshold_mb THEN
    PERFORM monitoring.post_alert(
      category := 'Storage Growth',
      severity := 'warning',
      message := pg_catalog.format('Database grew by %s MB (threshold %s MB)', growth_mb, cfg.storage_growth_threshold_mb),
      details := jsonb_build_object(
        'growth_mb', growth_mb,
        'threshold_mb', cfg.storage_growth_threshold_mb,
        'current_size_mb', pg_catalog.round((size_bytes / 1048576.0)::numeric, 2)
      )
    );
  END IF;
...
BEGIN
  ...
  IF very_slow_count > 0 THEN
    WITH top AS (
      SELECT queryid, calls, rows, pg_catalog.round(mean_time::numeric, 2) AS mean_time_ms,
             pg_catalog.left(query, 300) AS query
      FROM public.pg_stat_statements
      WHERE mean_time > cfg.very_slow_query_ms
      ORDER BY mean_time DESC
      LIMIT 5
    )
    SELECT pg_catalog.jsonb_agg(pg_catalog.to_jsonb(top)) INTO d FROM top;

    PERFORM monitoring.post_alert(
      category := 'Very Slow Queries',
      severity := 'critical',
      message := pg_catalog.format('%s queries exceed %sms (showing top 5)', very_slow_count, cfg.very_slow_query_ms),
      details := pg_catalog.jsonb_build_object('count', very_slow_count, 'threshold_ms', cfg.very_slow_query_ms, 'samples', d)
    );
  END IF;
END;
$$;
