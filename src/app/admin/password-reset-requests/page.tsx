...
export default function PasswordResetRequestsPage() {
  ...
  const loadRequests = useCallback(async () => {
    try {
      setLoading(true);
      const response = await fetch(`/api/admin/password-reset-requests?status=${statusFilter}&limit=100`, {
        credentials: 'include'
      });
  ...
  const confirmAction = async () => {
    ...
    try {
      ...
      if (actionType === 'delete') {
        const response = await fetch(`/api/admin/password-reset-requests?id=${selectedRequest.id}`, {
          method: 'DELETE',
          credentials: 'include',
          headers: {
        ...
        const data = await response.json();
        if (response.ok) {
          alert('Password reset request deleted successfully');
          await loadRequests();
        ...
        console.log('📤 Sending password reset request update:');
        ...
        // #region agent log
        fetch('http://127.0.0.1:7242/ingest/aa2d37a3-b977-45e9-919f-23aa5642fdcf',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'page.tsx:247',message:'Frontend: Request body before sending',data:{requestBody,actionType,userId,selectedRequestId:selectedRequest.id},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A,B,C'})}).catch(()=>{});
        // #endregion
        
        const response = await fetchWithCsrf('/api/admin/password-reset-requests', {
          method: 'PATCH',
          headers: {
        ...
        // #region agent log
        fetch('http://127.0.0.1:7242/ingest/aa2d37a3-b977-45e9-919f-23aa5642fdcf',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'page.tsx:260',message:'Frontend: Response received',data:{status:response.status,statusText:response.statusText,ok:response.ok},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A,B,C'})}).catch(()=>{});
        // #endregion
        ...
        if (response.ok) {
          ...
          // #region agent log
          fetch('http://127.0.0.1:7242/ingest/aa2d37a3-b977-45e9-919f-23aa5642fdcf',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'page.tsx:275',message:'Frontend: Request failed with error response',data:{status:response.status,error:data.error,details:data.details,validationIssues:data.validationIssues,requestBody},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A,B,C,D,E'})}).catch(()=>{});
          // #endregion
              ...
              <TableBody>
                {filteredRequests.map((request) => (
                  <TableRow key={request.id}>
                    ...
                    <TableCell>
                      <span className="text-sm">{request.schools?.name || 'N/A'}</span>
                    </TableCell>
                    <TableCell>
        ...
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>
              ...
              {actionType === 'delete' && 'Delete Password Reset Request'}
            </DialogTitle>
            <DialogDescription>
              ...
              {actionType === 'delete' && 'Are you sure you want to delete this request?'}
            </DialogDescription>
          </DialogHeader>
          {selectedRequest && (
            <div className="space-y-4 mt-4">
              {actionType === 'view' && (
                <div className="space-y-2">
                  ...
                  <div>
                    <Label>School</Label>
                    <p className="text-sm">{selectedRequest.schools?.name || 'N/A'}</p>
                  </div>
                  <div>
                    <Label>Status</Label>
