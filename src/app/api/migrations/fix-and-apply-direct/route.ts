...
/**
 * API endpoint to fix duplicate migration issue and apply the new migration directly
 */
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Step 2: Apply the new migration SQL directly
    ...
    if (!createResponse.ok) {
      // If RPC doesn't work, try using the Supabase CLI
      return NextResponse.json({
        success: false,
        message: 'Cannot create function via RPC, trying alternative method...',
    ...
    logger.error('Unexpected error in POST /api/migrations/fix-and-apply-direct', {
      endpoint: '/api/migrations/fix-and-apply-direct',
