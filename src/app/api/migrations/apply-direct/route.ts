...
/**
 * API endpoint to apply the migration directly using Supabase admin client
 */
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Step 2: Apply the new migration SQL
    ...
    // Execute the function creation using REST API
    ...
    if (!createResponse.ok) {
      ...
      // Let's try using the Supabase CLI or return instructions
      return NextResponse.json({
        success: false,
        message: 'Cannot create function via RPC',
        instructions: [
          'Please run the following SQL in Supabase SQL Editor:',
          createFunctionSQL,
          '',
          'Then call: SELECT apply_time_columns_fix();',
          '',
          'Then mark as applied:',
          "INSERT INTO supabase_migrations.schema_migrations(version, name, statements) VALUES ('20250111000001', 'fix_teacher_reports_time_columns', '') ON CONFLICT (version) DO NOTHING;"
        ]
      }, { status: 200 });
    }
    ...
    logger.error('Unexpected error in POST /api/migrations/apply-direct', {
      endpoint: '/api/migrations/apply-direct',
